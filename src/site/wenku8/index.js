...
async function get_volume_list(url) {
    ...
    return await jsdom_extra_1.fromURL(url)
        ...
        let { novel_cover, novel_desc, novel_status, novel_date, novel_publisher, } = await get_meta(url_data);
...
async function download(url) {
    ...
    let idx = 0;
    let path_novel = path.join(exports.PATH_NOVEL_MAIN, `${fs_iconv_1.trimFilename(novel.novel_title)}_(${novel.url_data.novel_id})`);
    let ret = await Promise
        ...
        return Promise
            ...
            let file = path.join(dirname, `${cid}_${fs_iconv_1.trimFilename(chapter.chapter_title)}\.${chapter.chapter_id}${ext}`);
            let dom = await jsdom_extra_1.fromURL(chapter.chapter_url);
            ...
            text = novel_text_1.default.trim(content.text())
                .replace(/ /g, ' ')
                .replace(/^[  \t]{4,}/gm, '　　')
                .replace(/^[  \t]+/gm, '　')
                .replace(/^(　+)[  \t]+/gm, '$1')
                .replace(/^　{2}/gm, '');
            text = novel_text_1.default.trim(text)
                .replace(/(@\}\})\n*(\{\{@)/g, '$1\n$2');
            let _idx = 0;
            for (let id in _c) {
                await image_1.download_image(_c[id], {
                    fromfile: file,
...
async function get_meta(url_data) {
    ...
    let dom = await jsdom_extra_1.fromURL(url);
    //novel_status
    //novel_cover
    //novel_desc
    ...
    dom.$('#content > div > table:eq(0) tr:eq(-1) > td').each(function (i, elem) {
        let t = dom.$(elem).text();
        if (t.match(/(?:状态|狀態)：\s*(.+)/)) {
            novel_status = novel_text_1.default.trim(RegExp.$1, {
                trim: true,
            });
        }
        else if (t.match(/(?:更新)：\s*(.+)/)) {
            novel_date = novel_text_1.default.trim(RegExp.$1, {
                trim: true,
            });
            novel_date = moment(novel_date).local();
        }
        else if (t.match(/(?:文库分类)：\s*(.+)/)) {
            novel_publisher = novel_text_1.default.trim(RegExp.$1, {
                trim: true,
            });
        }
    });
